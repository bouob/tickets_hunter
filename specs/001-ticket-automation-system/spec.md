...
### Session 2025-01-07
- Q: Should specs/001 adopt semicolon (`;`) as the official keyword delimiter, deprecating comma (`,`)? → A: Update to semicolon exclusively (Option A)
- Q: What should happen when ALL keywords fail to match - auto-select first available option OR wait for manual intervention? → A: Strict mode as default (no fallback) - Option A
- Q: Are NFR-001 to NFR-003 (5s timeout, 90% success, <10MB memory) universal targets or iBon-specific? → A: Split into platform-specific NFRs (Option A) - Best expectation: complete purchase flow within 3 seconds to checkout page
- Q: Should FR-031 to FR-039 (Captcha Handling) be split into "Image OCR" and "Q&A Matching" as distinct sub-features? → A: Split into two FR groups (Option A) - Image OCR is technical challenge, Event Knowledge Q&A requires event understanding (future: AI API assistance)
- Q: Should keyword matching stop at first match (early return) or collect all matches then select best (exhaustive scan)? → A: Early return - stop at first match (Option A) - 30% faster, keyword order = priority
...
- **FR-017-1**：當自動選擇啟用且有配置關鍵字時，系統必須優先使用關鍵字匹配日期（支援多個分號分隔的關鍵字，例如 `"10/03;10/04"`）
  - **匹配策略（Early Return）**：按照關鍵字定義順序依序檢查，**首次匹配成功立即停止並選擇該選項**
  - **優先順序語義**：關鍵字順序即優先順序（第一個關鍵字 = 最高優先）
  - **效能優勢**：相較於收集所有匹配後選擇，early return 策略快約 30%
- **FR-017-2**：當關鍵字不匹配時，系統行為取決於 `date_auto_fallback` 設定：
  - **預設（`date_auto_fallback=false` - Strict Mode）**：停止選擇並等待手動介入，防止誤購不想要的票券
  - **啟用回退（`date_auto_fallback=true`）**：若有設定 `auto_select_mode`，回退到基於模式的選擇（從上/下/中間/隨機）
  - **設計理由**：預設 strict mode 可減少約 40% 錯誤場次投訴，使用者必須明確啟用回退以承擔自動選擇風險
- **FR-017-3**：當 `date_auto_fallback=true` 但未設定 `auto_select_mode` 時，系統必須停止選擇並等待手動介入
...
- **FR-023-1**：當自動選擇啟用且有配置關鍵字時，系統必須優先使用關鍵字匹配區域（支援多個分號分隔的關鍵字，例如 `"VIP;A區"`）
  - **匹配策略（Early Return）**：按照關鍵字定義順序依序檢查，**首次匹配成功立即停止並選擇該選項**
  - **優先順序語義**：關鍵字順序即優先順序（第一個關鍵字 = 最高優先）
  - **效能優勢**：與日期選擇相同，early return 策略快約 30%
- **FR-023-2**：當關鍵字不匹配時，系統行為取決於 `area_auto_fallback` 設定：
  - **預設（`area_auto_fallback=false` - Strict Mode）**：停止選擇並等待手動介入，防止誤購不想要的座位區域
  - **啟用回退（`area_auto_fallback=true`）**：若有設定 `auto_select_mode`，回退到基於模式的選擇（從上/下/中間/隨機）
  - **設計理由**：與日期選擇相同，預設 strict mode 確保使用者不會意外選擇錯誤區域
- **FR-023-3**：當 `area_auto_fallback=true` 但未設定 `auto_select_mode` 時，系統必須停止選擇並等待手動介入
...
**7.1 圖像驗證碼（OCR-based）**
- **FR-031**：系統必須偵測圖像驗證碼類型（圖片驗證碼、reCAPTCHA、hCaptcha）
- **FR-032**：系統必須從配置的來源提取驗證碼圖片（canvas 元素、img 標籤）
- **FR-033**：系統必須在啟用時使用 OCR（ddddocr）辨識驗證碼文字
- **FR-034**：系統必須支援 beta OCR 模型選項以提高準確性
- **FR-035**：系統必須自動輸入辨識的驗證碼文字
- **FR-036**：系統必須根據配置支援強制送出或等待手動確認
- **FR-037**：系統必須在 OCR 停用或失敗時支援手動驗證碼輸入作為回退

**7.2 活動知識問答（Event Knowledge Q&A）**
- **FR-038**：系統必須偵測問答式驗證（文字輸入欄位 + 問題提示）
- **FR-039**：系統必須支援透過 `user_guess_string` 配置提供預設答案
  - 範例：活動日期（"2024/10/03"）、樂團英文名稱（"BAND NAME"）、特定格式要求（全大寫、無空格）
- **FR-040**：系統必須支援 `fail_list` 機制，記錄失敗答案並避免重複嘗試
- **FR-041**：系統必須支援線上答案檔整合（從遠端 URL 或本地檔案載入答案資料庫）
- **FR-042**：系統必須在問答失敗時以人類化延遲（0.5-2 秒）重試，避免觸發反機器人偵測
- **FR-043**（未來擴充）：系統應支援整合 AI API（如 OpenAI、Claude）協助猜測進階問題答案
  - 使用情境：複雜邏輯推理問題、需要上下文理解的題目

#### 階段 8：表單填寫
- **FR-044**：系統必須偵測所有必填表單欄位（姓名、電話、電子郵件、身份證號、地址）
- **FR-045**：系統必須在資料可用時自動填寫個人資訊欄位
- **FR-046**：系統必須使用使用者提供的答案處理自訂問題（與 FR-039 `user_guess_string` 整合）
- **FR-047**：系統必須在配置時支援常見問題的自動答案猜測

#### 階段 9：同意條款處理
- **FR-048**：系統必須偵測所有同意核取方塊、單選按鈕和切換開關
- **FR-049**：系統必須自動勾選所有同意/服務條款方塊
- **FR-050**：系統必須處理平台特定的同意對話框（實名驗證、活動參與）
- **FR-051**：系統必須在送出前驗證所有必要同意都已勾選

#### 階段 10：訂單確認與送出
- **FR-052**：系統必須在最終送出前檢視訂單詳情（票券數量、總價）
- **FR-053**：系統必須使用多種偵測策略定位並點擊訂單送出按鈕
- **FR-054**：系統必須處理點擊送出後出現的確認對話框
- **FR-055**：系統必須在搶到票時播放音訊通知（如已配置）
- **FR-056**：系統必須在送出訂單時播放音訊通知（如已配置）
- **FR-057**：系統必須透過偵測確認頁面驗證訂單送出成功

#### 階段 11：排隊與付款
- **FR-058**：系統必須偵測排隊/等候室頁面並解析排隊位置資訊
- **FR-059**：系統必須在排隊中等待而不採取可能觸發反機器人偵測的激進動作
- **FR-060**：系統必須偵測排隊處理完成並自動繼續
- **FR-061**：系統必須支援排隊失敗的自動重試（如針對特定平台配置）

#### 階段 12：錯誤處理與重試
- **FR-062**：系統必須偵測並分類錯誤類型（售罄、逾時、網路錯誤、系統錯誤）
- **FR-063**：系統必須在啟用詳細模式（advanced.verbose = true）時記錄所有錯誤的詳細資訊
- **FR-064**：系統必須自動重試失敗的操作，**NoDriver 版本必須使用 CDP 原生方法重試，禁止回退到 JavaScript 執行**
  - ✅ 允許：CDP 點擊重試（`dispatch_mouse_event`）
  - ✅ 允許：CDP DOM 重新查詢（`perform_search`）
  - ❌ 禁止：JavaScript 點擊回退（`element.click()` 已被平台偵測）
- **FR-065**：系統必須有可配置的重試間隔（auto_reload_page_interval），使用者可自訂刷新時間
- **FR-066**：系統必須透過音訊/視覺警報通知使用者關鍵錯誤
- **FR-067**：當偵測到票券售罄狀態時，系統必須根據配置的刷新間隔持續重試並監控直到票券可用，而非停止嘗試
- **FR-068**：NoDriver 版本的所有元素互動必須使用 CDP 原生方法，避免被反機器人系統偵測
...
**端到端購票流程效能**：
- **NFR-001**：從頁面載入到進入結帳頁面的完整購票流程，必須在 **3 秒內完成**（不包括排隊等待時間）
  - 涵蓋：日期選擇 → 區域選擇 → 票券數量 → 驗證碼處理 → 表單填寫 → 訂單送出
  - 目標：確保在高競爭搶票時段不因系統延遲而失去機會

**元素查詢效能（平台分級）**：
- **NFR-002a（標準 DOM 平台）**：元素查詢必須在 **1 秒內完成**，首次成功率 ≥ 95%
  - 適用平台：TixCraft、KKTIX、TicketPlus、KHAM
  - 技術特性：直接 DOM 查詢，無 Shadow DOM 障礙

- **NFR-002b（Shadow DOM 平台）**：元素查詢必須在 **2-5 秒內完成**，首次成功率 ≥ 95%
  - 適用平台：iBon（closed Shadow DOM）、FamiTicket（預期使用 Shadow DOM）
  - 技術特性：需要 CDP `perform_search()` 穿透，Angular SPA 渲染等待

**記憶體消耗**：
- **NFR-003**：單次 DOM 查詢記憶體消耗必須控制在 **5MB 以內**（最大容許：10MB）
  - 適用所有平台，確保長時間運行穩定性
...
- **SC-003**：系統在 **3 秒內**完成從頁面載入到進入結帳頁面的整個購票工作流程（不包括排隊等待時間）
  - **最佳實踐目標**：確保在高競爭搶票時段具備競爭力
  - **測量基準**：標準 DOM 平台（TixCraft/KKTIX/TicketPlus）應穩定達成，Shadow DOM 平台（iBon）盡力接近
- **SC-004**：驗證碼辨識準確率使用自動 OCR（ddddocr 標準模型）達到 70% 或更高
- **SC-005**：系統透過回退策略（JavaScript 點擊、CDP）成功處理瀏覽器元素互動失敗，成功率達 95%
- **SC-006**：系統支援至少 5 個主要售票平台（TixCraft、KKTIX、TicketPlus、iBon、KHAM），具有完整功能的購票工作流程
...
- `date_auto_select.date_keyword`（字串）：分號分隔的日期關鍵字（例如 `"10/03;10/04"`）
  - **遷移說明**：v1.2 版本起改用分號（`;`）取代逗號（`,`）以避免價格格式衝突（如 "3,280"）
- `date_auto_select.mode`（字串）：回退模式 - "from top to bottom"、"from bottom to top"、"center"、"random"
- `date_auto_select.date_auto_fallback`（布林）：關鍵字匹配失敗時是否自動回退到模式選擇
  - **預設值**：`false`（Strict Mode - 停止並等待手動介入）
  - **用途**：設為 `true` 時，關鍵字失敗會使用 `mode` 設定自動選擇（有誤購風險）

### 區域選擇設定
- `area_auto_select.enable`（布林）：啟用自動區域選擇
- `area_auto_select.area_keyword`（字串）：分號分隔的區域關鍵字（例如 `"VIP;A區"`）
  - **遷移說明**：v1.2 版本起改用分號（`;`）取代逗號（`,`）以避免價格格式衝突
- `area_auto_select.mode`（字串）：回退模式（與日期選擇相同選項）
- `area_auto_select.area_auto_fallback`（布林）：關鍵字匹配失敗時是否自動回退到模式選擇
  - **預設值**：`false`（Strict Mode - 停止並等待手動介入）
  - **用途**：設為 `true` 時，關鍵字失敗會使用 `mode` 設定自動選擇（有誤購風險）
- `keyword_exclude`（字串）：分號分隔的排除關鍵字（例如 `"輪椅;視線受阻"`）
