...
**效能目標**：
- **工作流程完成**：從頁面載入到訂單送出 <10 秒（不包括排隊等待）
- **關鍵字匹配**：第一選擇日期/區域選擇 90%+ 成功率
- **驗證碼辨識**：使用 ddddocr 達到 70%+ OCR 準確率
- **元素互動**：使用 JavaScript/CDP 回退達到 95%+ 成功率
...
2. **UserConfiguration（使用者配置）**（`settings.json`）
   ...
   - 日期/區域選擇（關鍵字、模式、排除項）
   - 驗證碼設定（OCR 啟用、beta、force_submit、image_source）
...
- 三層回退策略：
  - 第 1 層：支援多關鍵字的關鍵字匹配
  - 第 2 層：基於模式的回退（上/下/中/隨機）
  - 第 3 層：停止並等待手動介入（當 mode 未設定時）
- 選擇驗證
