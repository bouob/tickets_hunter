...
- **[遷移研究報告](chrome-nodriver-compatibility-research.md)** -- 完整相容性分析
...
| `evaluate` 回傳值 | RemoteObject，需 `parse_nodriver_result()` 解析 | 直接回傳實際值（字串/數字/bool），新程式碼停用 `parse_nodriver_result()` |
| 移除 handler | `tab.remove_handler(event, handler)` 逐一移除 | `tab.remove_handlers(event)` 移除**全部**同型別 handler |
