...
在「進階設定」頁面，Discord Webhook URL 下方有「**Discord 通知訊息**」欄位，Telegram Chat ID 下方有「**Telegram 通知訊息**」欄位。填入任何文字後，搶到票時就會送出你自訂的訊息；留空則維持原本的英文預設通知。
...
|------|----------|
| 簡單中文 | `已搶到票！請盡快到網頁上確認` |
| 多人共用頻道 | `@everyone 搶票成功，快去付款！` |
| 含 emoji 多行 | 搶到了!` + 換行 + `請盡速到網站確認` |

> **說明**：本功能為純文字，不支援時間、網址等動態變數。

**測試按鈕預覽**：按下 Discord / Telegram 旁的「測試」按鈕時，會直接送出你填入的訊息內容，方便在正式搶票前確認格式是否正確；欄位留空則送出預設的測試訊息。
