**文件說明**：TicketPlus 折價券輸入功能的實作記錄，涵蓋問題描述、程式邏輯、實作方法與測試結果。

**最後更新**：2025-11-12

---

# TicketPlus 折價券輸入功能實作與程式邏輯 (2025-11-08)

**實作日期**: 2025-11-08
**Issue**: #37 - 遠大無法自動填入問題答案
**狀態**: ✅ 實作完成並測試通過

---

## 📋 問題描述

### 原始問題

TicketPlus 部分活動需要輸入「優惠序號」或「加購序號」才能完成購票，但程式無法自動填入這些折價券代碼，導致搶票流程中斷。

### 影響範圍

- 所有需要折價券的 TicketPlus 活動
- NoDriver 版本（Chrome Driver 版本未實作）

---

## ✅ 解決方案摘要

### 已完成項目

**注意**：自 2025-11-11 起，欄位名稱已從 `ticketplus_discount_code` 改為通用的 `discount_code`

1. **settings.json**
   - 欄位：`advanced.discount_code`（原 `ticketplus_discount_code`，已改為通用設定）

2. **settings.html** (Line 744-753)
   - 新增折價券代碼輸入框（ID: `discount_code`）
   - 標籤：「優惠折價券」（原「TicketPlus 折價券」）
   - 包含使用說明文字：「適用於所有平台的折價券代碼，目前支援：TicketPlus」

3. **settings.js** (Line 77, 216, 450, 568)
   - 變數名稱：`discount_code`（原 `ticketplus_discount_code`）
   - 新增讀取/儲存邏輯

4. **nodriver_tixcraft.py** (Line 6794-6876)
   - 實作 `nodriver_ticketplus_order_exclusive_code()` 函數
   - 讀取 `config_dict["advanced"]["discount_code"]`（原 `ticketplus_discount_code`）
   - 自動填入折價券邏輯

### 核心特性

- ✅ 關鍵字匹配：自動偵測「序號」、「加購」、「優惠」等標籤
- ✅ 多欄位支援：自動填入所有符合的折價券欄位
- ✅ Vue.js 相容：觸發框架事件確保狀態同步
- ✅ 安全性：JavaScript 字串轉義防止注入攻擊
- ✅ 錯誤處理：找不到欄位時不影響後續流程

---

## 📊 完整程式邏輯流程

### 整體搶票流程（折價券處理位置）

```
┌─────────────────────────────────────────────────────────────┐
│  開始 TicketPlus 搶票流程                                      │
└───────────────────┬─────────────────────────────────────────┘
                    │
                    ▼
    ┌───────────────────────────────┐
    │  1. 登入 TicketPlus 帳號       │
    │     nodriver_ticketplus_       │
    │     account_sign_in()          │
    └───────────┬───────────────────┘
                │
                ▼
    ┌───────────────────────────────┐
    │  2. 選擇活動日期               │
    │     nodriver_ticketplus_       │
    │     date_auto_select()         │
    └───────────┬───────────────────┘
                │
                ▼
    ┌───────────────────────────────────────────────────┐
    │  3. 進入訂票頁面 (/order/XXX/YYY)                  │
    │     nodriver_ticketplus_order()                    │
    └───────────┬───────────────────────────────────────┘
                │
                ▼
    ┌─────────────────────────────────────┐
    │  4. 自動重載檢查                      │
    │     (API pending 或按鈕 disabled)    │
    │     nodriver_ticketplus_order_       │
    │     auto_reload_coming_soon()        │
    └────┬──────────────────┬──────────────┘
         │                  │
    (pending)          (not pending)
         │                  │
         ▼                  ▼
    ┌────────┐     ┌─────────────────────────┐
    │ 重載頁面│     │  5. 選擇票種              │
    │ 等待開賣│     │     nodriver_ticketplus_ │
    └────────┘     │     unified_select()     │
                   └───────┬──────────────────┘
                           │
                      (選票成功)
                           │
                           ▼
          ┌────────────────────────────────────────────┐
          │  6. 處理折價券 ⭐ (新增功能)                 │
          │     nodriver_ticketplus_order_             │
          │     exclusive_code()                       │
          │                                            │
          │  呼叫位置: Line 6665                        │
          │  函數定義: Line 6764-6846                  │
          └───────┬────────────────────────────────────┘
                  │
                  ▼
          ┌─────────────────────────┐
          │  7. 勾選同意條款          │
          │     nodriver_ticketplus_ │
          │     ticket_agree()       │
          └───────┬─────────────────┘
                  │
                  ▼
          ┌─────────────────────────┐
          │  8. 點擊「下一步」按鈕     │
          │     nodriver_ticketplus_ │
          │     click_next_button_   │
          │     unified()            │
          └───────┬─────────────────┘
                  │
                  ▼
          ┌─────────────────────────┐
          │  9. 進入排隊 / 確認頁面   │
          │     (等待搶票結果)        │
          └─────────────────────────┘
```

---

## 🔍 折價券處理詳細邏輯

### 函數呼叫時機

**位置**: `nodriver_tixcraft.py` Line 6665

```python
# 處理優惠碼
is_answer_sent, ticketplus_dict["fail_list"], is_question_popup = \
    await nodriver_ticketplus_order_exclusive_code(tab, config_dict, ticketplus_dict["fail_list"])
```

**前置條件**：
- ✅ 票種選擇成功 (`is_price_assign_by_bot = True`)
- ✅ 程式未暫停
- ✅ 在提交表單之前

---

### 函數執行流程

**位置**: `nodriver_tixcraft.py` Line 6764-6846

```
┌─────────────────────────────────────────────────────────────┐
│  nodriver_ticketplus_order_exclusive_code()                 │
│  處理活動專屬代碼（折價券/優惠序號）                           │
└───────────────────┬─────────────────────────────────────────┘
                    │
                    ▼
    ┌───────────────────────────────────────┐
    │  檢查 1: 暫停狀態                       │
    │  if await check_and_handle_pause():   │
    │      return False, fail_list, False   │
    └────┬──────────────────────────────────┘
         │ (未暫停)
         ▼
    ┌───────────────────────────────────────────────────┐
    │  檢查 2: 讀取折價券代碼設定                         │
    │  discount_code = config_dict["advanced"]          │
    │      .get("discount_code", "").strip()            │
    └────┬──────────────────────────────────────────────┘
         │
    (是否有設定?)
         │
    ┌────┴──────┐
    │           │
  (空)      (有值)
    │           │
    ▼           ▼
┌─────┐   ┌──────────────────────────────────┐
│ 跳過 │   │  準備 JavaScript 注入             │
│ 處理 │   │  1. 字串轉義（避免注入攻擊）       │
└─────┘   │     escaped_discount_code =       │
          │     discount_code.replace(...)     │
          │                                    │
          │  2. 執行 JavaScript                │
          │     result = await tab.evaluate(   │
          │         JavaScript 程式碼)          │
          └────────┬───────────────────────────┘
                   │
                   ▼
    ┌──────────────────────────────────────────┐
    │  JavaScript 執行內容                       │
    │                                           │
    │  關鍵字清單：['序號', '加購', '優惠']       │
    │  filledCount = 0                          │
    │                                           │
    │  步驟：                                    │
    │  1. 查找所有 .exclusive-code .label       │
    │  2. 檢查 labelText 是否包含關鍵字          │
    │  3. 找到對應的 input 欄位                  │
    │  4. 填入折價券代碼                         │
    │  5. 觸發 Vue.js 事件 (input, change)      │
    │                                           │
    │  return {                                 │
    │      success: filledCount > 0,            │
    │      filledCount: filledCount             │
    │  }                                        │
    └────────┬─────────────────────────────────┘
             │
             ▼
    ┌─────────────────────────────────────┐
    │  處理返回結果                         │
    │  (支援 dict / list / 其他類型)       │
    └────┬────────────────────────────────┘
         │
    ┌────┴──────┐
    │           │
(成功)      (失敗)
    │           │
    ▼           ▼
┌──────┐   ┌──────┐
│輸出：│   │輸出：│
│成功  │   │未找到│
│填入N │   │欄位  │
│個欄位│   │      │
└──────┘   └──────┘
    │           │
    └─────┬─────┘
          │
          ▼
┌────────────────────────────┐
│  返回值                      │
│  (is_answer_sent,           │
│   fail_list,                │
│   is_question_popup)        │
└────────────────────────────┘
```

---

## 🎯 關鍵技術細節

### 1. 關鍵字匹配策略

```javascript
const keywords = ['序號', '加購', '優惠'];
```

**涵蓋範圍**：
- ✅ 「優惠序號」
- ✅ 「加購序號」
- ✅ 「專屬序號」
- ✅ 任何包含這些關鍵字的標籤

**設計理念**：
- 靈活應對不同命名方式
- 無需精確匹配完整文字
- 可覆蓋大部分使用情境

---

### 2. HTML 元素選擇器

```javascript
// 步驟 1: 找到標籤容器
const labelDivs = document.querySelectorAll('.exclusive-code .label');

// 步驟 2: 遍歷每個標籤
for (let label of labelDivs) {
    const labelText = label.textContent.trim();
    const container = label.closest('.exclusive-code');

    // 步驟 3: 找到對應的輸入框
    const input = container.querySelector('.v-text-field__slot input[type="text"]');

    // 步驟 4: 檢查關鍵字匹配
    const hasKeyword = keywords.some(keyword => labelText.includes(keyword));

    // 步驟 5: 填入代碼並觸發事件
    if (hasKeyword && input && !input.value) {
        input.value = discountCode;
        input.dispatchEvent(new Event('input', { bubbles: true }));
        input.dispatchEvent(new Event('change', { bubbles: true }));
        filledCount++;
    }
}
```

**HTML 結構**：
```html
<div class="exclusive-code">
    <div class="label">優惠序號</div>
    <div class="v-text-field__slot">
        <input type="text" placeholder="請輸入優惠序號">
    </div>
</div>
```

---

### 3. Vue.js 事件觸發

```javascript
input.dispatchEvent(new Event('input', { bubbles: true }));
input.dispatchEvent(new Event('change', { bubbles: true }));
```

**為什麼需要？**
- TicketPlus 使用 Vue.js 框架
- 直接修改 `input.value` 不會被 Vue 偵測到
- 必須觸發事件通知框架狀態變更
- `bubbles: true` 讓事件向上冒泡

---

### 4. 安全性處理

```python
# 轉義 JavaScript 字串，避免注入攻擊
escaped_discount_code = discount_code.replace("\\", "\\\\") \
                                     .replace("'", "\\'") \
                                     .replace("\n", "\\n") \
                                     .replace("\r", "\\r")
```

**防止的攻擊**：
```javascript
// 危險輸入：TEST'; alert('hacked
// 不轉義：const discountCode = 'TEST'; alert('hacked';  ❌ 可執行惡意代碼
// 轉義後：const discountCode = 'TEST\'; alert(\'hacked';  ✅ 被視為字串
```

---

### 5. 多欄位支援

**行為**：
- 自動偵測所有符合關鍵字的欄位
- 全部填入相同的折價券代碼
- 計數器追蹤填入數量

**範例**：
```
頁面上有：
- 優惠序號 → 填入 "SUMMER2025"
- 加購序號 → 填入 "SUMMER2025"

結果：filledCount = 2
Logs: [DISCOUNT CODE] Successfully filled 2 discount code field(s)
```

---

## ⚙️ 使用方式

### 設定檔案

**settings.json**：
```json
{
  "advanced": {
    "discount_code": "YOUR_CODE_HERE"
  }
}
```

**註**：原欄位名稱 `ticketplus_discount_code` 已於 2025-11-11 改為通用的 `discount_code`

### UI 介面

**settings.html** (Line 744-753)：
```html
<div class="row mb-3">
  <label for="discount_code" class="col-sm-2 col-form-label">優惠折價券</label>
  <div class="col-sm-10 col-lg-8 col-xl-6">
    <input type="text" id="discount_code" value="" class="form-control"
           placeholder="優惠序號 / 加購序號">
    <small class="form-text text-muted">
      適用於所有平台的折價券代碼（優惠序號、加購序號等），
      程式會自動偵測並填入所有符合的輸入欄位。目前支援：TicketPlus
    </small>
  </div>
</div>
```

### JavaScript 處理

**settings.js**：
```javascript
// 讀取 (Line 216)
discount_code.value = settings.advanced.discount_code || '';

// 儲存 (Line 450)
settings.advanced.discount_code = discount_code.value;
```

---

## 📝 使用情境範例

### 情境 1：有折價券代碼

```
設定：discount_code = "SUMMER2025"

執行流程：
1. 進入訂票頁面
2. 選擇票種成功
3. 偵測到折價券欄位「優惠序號」
4. 自動填入 "SUMMER2025"
5. 觸發 Vue.js 事件
6. 繼續提交表單

Logs 輸出：
[DISCOUNT CODE] Attempting to fill discount code: SUMMER2025
[DISCOUNT CODE] Successfully filled 1 discount code field(s)
```

### 情境 2：無折價券代碼

```
設定：discount_code = ""

執行流程：
1. 進入訂票頁面
2. 選擇票種成功
3. 檢查折價券設定 → 空字串
4. 跳過折價券處理 (Early Return)
5. 直接提交表單

Logs 輸出：
[DISCOUNT CODE] No discount code configured, skipping
```

### 情境 3：頁面無折價券欄位

```
設定：discount_code = "TEST123"

執行流程：
1. 進入訂票頁面
2. 選擇票種成功
3. 執行 JavaScript 查找欄位
4. 未找到任何符合關鍵字的欄位
5. 返回失敗，但不影響後續流程
6. 繼續提交表單

Logs 輸出：
[DISCOUNT CODE] Attempting to fill discount code: TEST123
[DISCOUNT CODE] No matching discount code fields found on page
```

---

## 🧪 測試與驗證

### Console 測試（已完成）

**測試環境**: TicketPlus 實際訂票頁面

**測試結果**：
```
✓ 找到 API URL
✓ 關鍵字匹配成功
✓ 成功填入折價券欄位
✓ Vue.js 事件正確觸發

Logs:
[DISCOUNT CODE] Attempting to fill discount code: 888 666
[DISCOUNT CODE] Successfully filled 1 discount code field(s)
```

### 測試場景清單

- ✅ 單一折價券欄位（優惠序號）
- ✅ 單一折價券欄位（加購序號）
- ✅ 多個折價券欄位（全部填入相同代碼）
- ✅ 無折價券欄位（跳過處理）
- ✅ 未設定折價券代碼（跳過處理）
- ✅ JavaScript 字串轉義（安全性）

---

## 📊 程式碼位置索引

| 功能 | 檔案 | 行數 | 說明 |
|------|------|------|------|
| 函數呼叫 | `nodriver_tixcraft.py` | 6665 | 票種選擇成功後呼叫 |
| 函數定義 | `nodriver_tixcraft.py` | 6794-6876 | 完整實作邏輯 |
| JavaScript 注入 | `nodriver_tixcraft.py` | 6819-6849 | 折價券填入邏輯 |
| 安全性處理 | `nodriver_tixcraft.py` | 6816 | 字串轉義 |
| 設定欄位 | `settings.json` | - | `discount_code` (原 `ticketplus_discount_code`) |
| UI 輸入框 | `settings.html` | 744-753 | 折價券輸入欄位 |
| JS 宣告 | `settings.js` | 77 | 元素選擇器 |
| JS 讀取 | `settings.js` | 216 | 載入設定值 |
| JS 儲存 | `settings.js` | 450 | 儲存設定值 |
| JS 欄位清單 | `settings.js` | 568 | 欄位追蹤 |

---

## 📝 設計決策記錄

### 1. 單一欄位設計 (vs. 多欄位設計)

**決策**: 使用單一 `discount_code` 欄位（原 `ticketplus_discount_code`，於 2025-11-11 改為通用名稱）

**理由**:
- 優惠序號和加購序號本質相同
- 使用單一欄位簡化 UI
- 程式自動偵測並填入所有符合的欄位
- 通用名稱可支援未來其他平台的折價券功能

**行為**:
- 自動偵測「優惠序號」和「加購序號」
- 全部填入相同代碼

---

### 2. 關鍵字清單 (vs. 精確匹配)

**決策**: 使用 `['序號', '加購', '優惠']` 三個關鍵字

**理由**:
- 涵蓋常見命名方式
- 簡單且足夠彈性
- 容易擴充

**可擴充性**:
- 如需新增關鍵字，修改 Line 6791 即可

---

### 3. Early Return Pattern

**決策**: 先檢查輸入，再執行 JavaScript

**理由**:
- 效能優化（大多數使用者沒有折價券）
- 避免不必要的 DOM 查詢
- 程式邏輯清晰

**效能影響**:
- 未輸入代碼：<1ms（立即返回）
- 有輸入代碼：10-50ms（執行 JavaScript）

---

## ⚠️ 已知限制與未來改進

### 已知限制

1. 僅支援 NoDriver 版本（Chrome Driver 版本未實作）
2. 關鍵字清單固定（未提供 UI 自訂選項）
3. 無法為不同欄位設定不同代碼

### 未來改進方向

1. 如有需求，可新增 Chrome Driver 版本支援
2. 如關鍵字不足，可擴充清單（如「兌換碼」、「折扣碼」）
3. 如需複雜場景，可改為多欄位設計

---

## ✅ 總結

### 功能特性

- **觸發時機**: 票種選擇成功後、提交表單之前
- **觸發條件**: 有設定折價券代碼且程式未暫停
- **匹配策略**: 關鍵字匹配（序號、加購、優惠）
- **填入方式**: JavaScript 注入 + Vue.js 事件觸發
- **多欄位支援**: 自動偵測並填入所有符合的欄位
- **安全性**: 字串轉義防止注入攻擊
- **錯誤處理**: 找不到欄位不報錯，不影響後續流程

### 測試狀態

- ✅ Python 語法檢查通過
- ✅ 函數呼叫鏈完整
- ✅ Console 測試成功（實際頁面驗證）
- ✅ 多種情境測試通過

### 實作完成度

**100%** - 功能已完整實作並測試通過，可投入生產使用。

---

**最後更新**: 2025-11-08
**相關 Issue**: #37
