...
├── nodriver_kham_main                   # 行 17425 ✅ (UDN: 行 17508-17523, 17669-17688)
├── nodriver_kham_login                  # 行 15956 ✅
├── nodriver_kham_date_auto_select       # 行 16401 ✅ (Feature 003: Early Return + Fallback)
├── nodriver_kham_go_buy_redirect        # 行 16130 ✅
├── nodriver_kham_product                # 行 16381 ✅
├── nodriver_kham_area_auto_select       # 行 16790 ✅ (Feature 003: Early Return + Fallback)
├── nodriver_kham_switch_to_auto_seat    # 行 16890 ✅
├── nodriver_kham_performance            # 行 17336 ✅
├── nodriver_kham_keyin_captcha_code     # 行 16682 ✅
├── nodriver_kham_auto_ocr               # 行 17215 ✅
├── nodriver_kham_captcha                # 行 17292 ✅
...
├── nodriver_kham_seat_main              # 行 19210 ✅ 座位選擇主流程
├── nodriver_kham_seat_auto_select       # 行 18845 ✅ 座位自動選擇核心
└── nodriver_kham_seat_type_auto_select  # 行 18480 ✅ 票種自動選擇
```

**UDN 專屬說明** (Feature 010: 2025-12-17)：
- UDN 與 KHAM 共用 UTK 後端，座位選擇邏輯完全複用
- UTK0205 座位選擇：行 17508-17523（`[UDN SEAT]` 日誌標記）
- UTK0204 票區選擇：行 17669-17688（`[UDN AREA]` 日誌標記）
...
#### util.py 共用函數 (v2025.12.18 新增)
```
選擇模式與關鍵字處理
├── get_target_index_by_mode             # 行 1342 ✅ 選擇模式索引計算（統一 8 個重複實作）
├── get_target_item_from_matched_list    # 行 1375 ✅ 從匹配列表取得目標項目
├── get_debug_mode                       # 行 1411 ✅ 安全讀取 debug 模式設定
├── parse_keyword_string_to_array        # 行 1439 ✅ 統一 JSON 關鍵字解析
└── reset_row_text_if_match_keyword_exclude # 行 1125 ✅ 排除關鍵字匹配檢查

OCR 與驗證碼
├── ddddocr_image_to_text                # 行 680
└── get_ocr_answer                       # 行 770

音訊與通知
├── play_mp3_async                       # 行 236
├── send_discord_webhook                 # 行 970
└── send_discord_webhook_async           # 行 1020
```

**v2025.12.18 重構說明**：
- `get_target_index_by_mode()`: 統一 UDN、iBon、FamiTicket 等 8 個重複的選擇模式邏輯
- `get_debug_mode()`: 替換 28 個直接索引存取，避免 KeyError
- `parse_keyword_string_to_array()`: 替換 9 個 json.loads 模式，簡化約 73 行代碼
