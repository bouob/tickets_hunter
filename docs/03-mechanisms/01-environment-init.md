...
`cli()` 使用 `argparse` 解析命令列參數，支援以下覆寫項：
...
`cli()` 最終呼叫 `asyncio.run(main(args))` 啟動非同步主流程。

#### 1.2 設定檔載入

`get_config_dict(args)` 負責：
...
**實作位置**：`src/nodriver_common.py`（get_config_dict）

### 2. 瀏覽器啟動

#### 2.1 Chrome 可用性檢查

`get_extension_config()` 在啟動前先確認 Chrome 是否可用：
...
**專家模式**（`CLOUDFLARE_ENABLE_EXPERT_MODE = False`，定義於 `src/nodriver_common.py`）：
啟用時額外加入 `--no-sandbox` 與 `--disable-web-security`，可增強繞過能力但提高被偵測風險。

**實作位置**：`src/nodriver_common.py`（get_nodriver_browser_args）

#### 2.3 Config 組裝與瀏覽器啟動

`get_extension_config()`（`src/nodriver_common.py`）組裝 `zendriver.Config` 物件，支援三種模式：
...
**實作位置**：`src/nodriver_tixcraft.py`（main）；`get_extension_config` / `nodriver_overwrite_prefs` / `nodriver_resize_window` 位於 `src/nodriver_common.py`

### 3. 網路封鎖清單

`nodrver_block_urls()` 透過 CDP `network.set_blocked_ur_ls()` 封鎖不必要的網路請求，分為三類：
...
**實作位置**：`src/nodriver_tixcraft.py`（nodrver_block_urls）
...
**進階暫停輔助函式**（`src/nodriver_common.py`）：
- `sleep_with_pause_check()` — 延遲前檢查暫停
- `evaluate_with_pause_check()` — JS 執行前檢查暫停

### 5. 設定熱更新

`reload_config()` 在主迴圈中持續監控 `settings.json` 的修改時間，當檔案變更時自動重新載入以下欄位：
...
| 檔案 | 說明 |
|------|------|
| `src/nodriver_tixcraft.py` | 主迴圈與 URL 路由（`cli()` / `main()` 進入點、`reload_config`、`nodrver_block_urls`） |
| `src/nodriver_common.py` | 共用瀏覽器基礎設施（Config 組裝、瀏覽器參數、暫停機制） |
