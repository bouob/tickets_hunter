...
### 通用填寫函式 `nodriver_fill_verify_form()`（`src/platforms/tixcraft.py`）
...
| 平台 | 函式名稱 | 檔案 | 填寫內容 | 特殊機制 |
|------|----------|------|----------|----------|
| TixCraft | `nodriver_fill_verify_form` | `platforms/tixcraft.py` | 驗證問題答案 | 支援 Enter 鍵或按鈕提交 |
| TixCraft | `nodriver_tixcraft_keyin_captcha_code` | `platforms/tixcraft.py` | 圖形驗證碼 | OCR 辨識後填入 |
| KKTIX | 整合於 `nodriver_kktix_assign_ticket_number` | `platforms/kktix.py` | 驗證問題答案 | 人類化延遲 0.3-1 秒，模擬逐字輸入 |
| iBon | `nodriver_ibon_fill_verify_form` | `platforms/ibon.py` | 驗證問題答案 | 支援雙輸入框模式（多題） |
| iBon | `nodriver_ibon_keyin_captcha_code` | `platforms/ibon.py` | 圖形驗證碼 | OCR 辨識後填入 |
| FamiTicket | `nodriver_fami_verify` | `platforms/famiticket.py` | 驗證問題答案 | 使用 `#verifyPrefAnswer` 選擇器 |
| KHAM | `nodriver_kham_keyin_captcha_code` | `platforms/kham.py` | 圖形驗證碼 | OCR 辨識後填入 |
| TicketPlus | `nodriver_ticketplus_account_auto_fill` | `platforms/ticketplus.py` | 登入帳號資訊 | 帳號密碼自動填入登入表單 |

---

## 平台關鍵差異

### KKTIX — 人類化填寫與 Angular 整合
...
### TicketPlus — 登入表單自動填寫（`nodriver_ticketplus_account_auto_fill`）
