...
**文件說明**：說明搶票系統的身份認證機制、登入流程與認證狀態維持策略
**最後更新**：2025-12-20
...
**認證機制**：
- ⚠️ **半自動登入**（需使用者協助完成 reCAPTCHA）
- 程式自動填入帳號密碼
- reCAPTCHA 驗證需使用者手動完成
- 使用者完成 reCAPTCHA 後需手動點擊登入按鈕

**登入流程**：
1. 程式自動開啟 UDN 登入對話框
2. 程式自動填寫帳號和密碼
3. ⚠️ **使用者需手動完成 reCAPTCHA 驗證**
4. ⚠️ **使用者需手動點擊「登入」按鈕**
5. 程式偵測到登入成功後自動繼續
...
{
  "advanced": {
    "udn_account": "your@email.com",
    "udn_password": "your_password"
  }
}
```

**程式碼位置**：
- `src/nodriver_tixcraft.py`: 行 17407-17835 (UDN 登入處理)
- 日誌標記：`[UDN LOGIN]`

**使用者須知**：
1. 程式會自動填寫帳號密碼，請在瀏覽器中完成 reCAPTCHA 圖片驗證
2. 選擇正確的圖片（公車、腳踏車、紅綠燈等）後點擊登入按鈕
3. 程式會自動偵測登入狀態並繼續搶票流程
4. 如果登入失敗，程式會在下一輪重試

**技術說明**：
- UDN 使用 Google reCAPTCHA v2 圖片驗證進行人機驗證
- reCAPTCHA 圖片驗證無法透過程式自動完成（設計如此）
- 這是平台安全措施，非程式限制
