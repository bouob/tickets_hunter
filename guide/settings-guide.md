...
#### 日期自動遞補（date_auto_fallback）⭐ 新功能
類型：布林值 | 預設：**false**（嚴格模式）
...
| 值 | 中文說明 | 行為描述 |
|---|---------|---------|
| **false** | **嚴格模式**（預設） | 關鍵字全失敗時**停止執行**，等待手動介入 |
| true | 自動遞補模式 | 關鍵字全失敗時**自動遞補**，使用「日期排序方式」選擇可用日期 |
...
| 使用情境 | 建議設定 | 原因 |
|---------|---------|------|
| 只想買特定日期（如：11/16） | `false` | 避免買到錯誤日期 |
| 任何日期都可以接受 | `true` | 確保能搶到票 |
| 混合策略 | 根據重要性決定 | 日期重要用 `false`，不重要用 `true` |

**實戰範例**：
```
情境：只想買 11/16 的票
設定：date_keyword = "11/16"
...
|---|---------|---------|
| **false** | **嚴格模式**（預設） | 關鍵字全失敗時**停止執行**，等待手動介入 |
| true | 自動遞補模式 | 關鍵字全失敗時**自動遞補**，使用「區域排序方式」選擇可用區域 |
...
| 使用情境 | 建議設定 | 原因 |
|---------|---------|------|
| 只想買特定區域（如：搖滾區） | `false` | 避免買到錯誤區域 |
| 任何座位都可以接受 | `true` | 確保能搶到票 |
| 混合策略 | `false` | 座位通常比日期更重要，建議保守 |
...
```
情境：日期不重要，但座位很重要
設定：date_auto_fallback = true   (日期可以自動選)
      area_auto_fallback = false  (座位必須嚴格匹配)
