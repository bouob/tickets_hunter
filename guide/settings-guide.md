...
**相容性說明**：
- 舊版或外部工具若寫入未支援的值，設定頁會自動回退為「繁體中文」
- 目前僅設定頁 UI 支援繁體中文與英文切換
- 其他執行邏輯中的多語言偵測能力（例如售完、尚未開賣）仍可獨立支援更多語言，不受此欄位限制
...
**範例值**：2025/12/25 14:00:00

**使用情境**：
- 已知演唱會下午2點開賣
- 提前開啟程式並導航到售票頁面
- 程式會在 2025/12/25 14:00:00 自動重整頁面並開始搶票
...
在圖形介面中顯示為：**不足張數仍購買**（進階設定）

| 設定張數 | 可選張數 | 關閉（預設） | 開啟 |
|---------|---------|-------------|------|
| 4 | 4、3、2、1 | 選 4 | 選 4 |
| 4 | 3、2、1 | 不送出，重新整理等待 | 選 3 |
| 2 | 1 | 不送出，重新整理等待 | 選 1 |
| 2 | 無 | 不送出，重新整理等待 | 不送出，重新整理等待 |

**注意**：開啟後可能買到少於原本設定的張數，請只在您能接受較少張數時使用。
...
**設定方式**：
- 留空：沿用內建預設隨機 `240-420` 秒
- 填入整數（例如 `30`、`60`）：固定等待指定秒數後重試

**說明**：
- 只影響拓元家族白畫面暫時鎖定等待時間
- 不影響 Ticketmaster
- 若設定過短，可能仍在封鎖期間內，重試後還是會回到白畫面
...
「多開」= 同時跑多個搶票實例，涵蓋三種情境：不同平台同時搶、同平台不同活動、不同帳號分散風險。

### 用圖形介面管理

1. 上方「實例設定檔」列預設有 `default`（即 `settings.json`）。
...
- **圖形介面（推薦）**：上方「實例設定檔」列點「＋ 新增」選平台建立 profile，每個 profile 是獨立設定檔，可各自「搶票」成一個實例。詳見[多開實例](#多開實例)。
- **命令列**：複製 settings.json 並改名，啟動時指定：`nodriver_tixcraft.exe --input settings_A.json`

⚠️ 同一活動、同帳號多開會被平台踢登入（session 互踢）；安全用法是「一個實例一個帳號」。
