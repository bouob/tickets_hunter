...
- **改進自動遞補邏輯說明**：完善流程圖表，清楚顯示當關鍵字無法匹配時的決策路徑
- **改善術語表達**：將技術術語「早期返回模式」改為更直觀的「依序選擇機制」，提升文件易讀性
- **用戶介面術語對齊**：將所有技術參數名稱替換為設定介面中的中文標籤，確保文件與介面一致
- **增強設定介面**：改進 CSS 深色模式支援和按鈕互動效果
- **清理過時設定**：為 `settings_old.py` 添加棄用警告，引導使用者使用現代化網頁介面

### 版本更新

- 將版本號更新至 2025.11.03（同步至 6 個檔案）
